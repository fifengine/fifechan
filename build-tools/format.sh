...
if [[ -z "${CI:-}" && -z "${GITHUB_ACTIONS:-}" ]]; then
  ...
  find src tests include examples tests -type f \( -name "*.h" -o -name "*.hpp" -o -name "*.cpp" \) -exec dos2unix {} \;
fi

find src tests include examples tests -type f \( -name "*.h" -o -name "*.hpp" -o -name "*.cpp" \) -exec "$CLANG_FORMAT" -i -style=file {} \;
