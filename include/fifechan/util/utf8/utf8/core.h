...
// The typedefs for 8-bit, 16-bit and 32-bit code units
    ...
    namespace internal
    {
        ...
        utfchar16_t const LEAD_SURROGATE_MIN  = 0xd800u;
        utfchar16_t const LEAD_SURROGATE_MAX  = 0xdbffu;
        utfchar16_t const TRAIL_SURROGATE_MIN = 0xdc00u;
        utfchar16_t const TRAIL_SURROGATE_MAX = 0xdfffu;
        utfchar16_t const LEAD_OFFSET         = 0xd7c0u;  // LEAD_SURROGATE_MIN - (0x10000 >> 10)
        utfchar32_t const SURROGATE_OFFSET = 0xfca02400u; // 0x10000u - (LEAD_SURROGATE_MIN << 10) - TRAIL_SURROGATE_MIN

        // Maximum valid value for a Unicode code point
        utfchar32_t const CODE_POINT_MAX = 0x0010ffffu;
        ...
        {
            utfchar8_t const lead = utf8::internal::mask8(*lead_it);
            if (lead < 0x80)
                return 1;
            else if ((lead >> 5) == 0x6)
                return 2;
        ...
        template <typename octet_iterator>
        utf_error increase_safely(octet_iterator& it, octet_iterator const end)
        {
            if (++it == end)
...
#undef UTF8_CPP_INCREASE_AND_RETURN_ON_ERROR
        ...
        utf_error validate_next(octet_iterator& it, octet_iterator end, utfchar32_t& code_point)
        {
            ...
            int const length = utf8::internal::sequence_length(it);
        ...
        utf_error validate_next16(word_iterator& it, word_iterator end, utfchar32_t& code_point)
        {
            ...
            utfchar16_t const first_word = *it++;
            if (!is_surrogate(first_word)) {
                ...
                if (it == end)
                    ...
                    utfchar16_t const second_word = *it++;
    ...
    // Byte order mark
    utfchar8_t const bom[] = {0xef, 0xbb, 0xbf};
    ...
    inline char const * find_invalid(char const * str)
    {
        char const * end = str + std::strlen(str);
        return find_invalid(str, end);
    }

    inline std::size_t find_invalid(std::string const & s)
    {
    ...
    inline bool is_valid(char const * str)
    {
    ...
    inline bool starts_with_bom(std::string const & s)
    {
