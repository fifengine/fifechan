...
using tests::integration::opengl_sdl::widgets::Application;
using tests::integration::opengl_sdl::widgets::DemoListModel;
...
void Application::init_GUI(int width, int height)
{
    ...
    auto iconPtr = std::make_unique<fcn::Icon>(image);
    ...
    auto imageButtonPtr = std::make_unique<fcn::ImageButton>("gui-chan.bmp");
    ...
    logoImage              = fcn::Image::load("gui-chan.bmp");
    auto guisanLogoIconPtr = std::make_unique<fcn::Icon>(logoImage);
    ...
    guisanLogoIcon = dynamic_cast<fcn::Icon*>(fcnWindow->getChild(0));
